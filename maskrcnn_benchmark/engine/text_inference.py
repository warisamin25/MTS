    ...
    for i, batch in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            ...
            if predictions is not None:
                ...
                used_size = 0
                global_predictions = predictions[0]
                global_predictions = [o.to(cpu_device)
                                      for o in global_predictions]
                for batch in range(len(predictions[0])):
                    size = len(predictions[0][batch].area())
                    char_mask = char_predictions['char_mask'][used_size:used_size + size, :, :, :]
                    boxes = predictions[0][batch].bbox
                    seq_words = char_predictions['seq_outputs'][used_size:used_size + size]
                    seq_scores = char_predictions['seq_scores'][used_size:used_size + size]
                    detailed_seq_scores = char_predictions['detailed_seq_scores'][used_size:used_size + size]

                    results_dict.update(
                        {image_paths[batch]: [global_predictions[batch], char_mask,
                                          boxes, seq_words, seq_scores, detailed_seq_scores]}
                    )
    return results_dict
...
def mask2polygon(mask, box, im_size, threshold=0.5, output_folder=None):
    ...
    if 'total_text' in output_folder or 'cute80' in output_folder:
        ...
        pts = get_tight_rect(
            corners, box[0], box[1], image_height, image_width, 1)
...
def format_output(out_dir, boxes, img_name):
    res = open(os.path.join(out_dir, 'res_' +
                            img_name.split('.')[0] + '.txt'), 'wt')
    # char score save dir
    ...
    for i, box in enumerate(boxes):
        ...
        if 'total_text' in out_dir or 'cute80' in out_dir:
            ...
            box = ','.join([str(x) for x in box[:4]]) + ';' + ','.join([str(x) for x in box[4:4+int(
                box[-1])]]) + ';' + ','.join([str(x) for x in box[4+int(box[-1]):-3]]) + ',' + save_name
        else:
...
def prepare_results_for_evaluation(predictions, output_folder, model_name, vis=False):
    ...
    for image_path, prediction in predictions.items():
        im_name = image_path.split('/')[-1]
        global_prediction, char_mask, boxes_char, seq_words, seq_scores, detailed_seq_scores = prediction[
            0], prediction[1], prediction[2], prediction[3], prediction[4], prediction[5]
        ...
        for k, box in enumerate(boxes):
            box = list(map(int, box))
            mask = masks[k, 0, :, :]
            polygon = mask2polygon(mask, box, img.size,
                                   threshold=0.5, output_folder=output_folder)
            if polygon is None:
            ...
            seq_score = sum(seq_char_scores) / float(len(seq_char_scores))
            detailed_seq_score = detailed_seq_scores[k]
            detailed_seq_score = np.squeeze(
                np.array(detailed_seq_score), axis=1)
            if 'total_text' in output_folder or 'cute80' in output_folder:
                result_log = [int(x * 1.0) for x in box[:4]] + polygon + [word] + [seq_word] + [score] + [
                    rec_score] + [seq_score] + [char_score] + [detailed_seq_score] + [len(polygon)]
            else:
                result_log = [int(x * 1.0) for x in box[:4]] + polygon + [word] + [seq_word] + [
                    score] + [rec_score] + [seq_score] + [char_score] + [detailed_seq_score]
            result_logs.append(result_log)
        if vis:
            colors = creat_color_map(37, 255)
            visualization(img, polygons, char_polygons,
                          words, resize_ratio, colors)
...
def inference(
    ...
    predictions_path = os.path.join(
        output_folder, model_name + '_predictions.pth')
    ...
    prepare_results_for_evaluation(
        predictions, output_folder, model_name, vis=cfg.TEST.VIS)
